...
def write_json_to_file(json_obj, out_file:str):
    ''' json数据写入文件
    @json_obj: 待写入数据
    @out_file: 输出文件
    '''
    ...
    print(f"write_json_to_file > 数据已写入文件: {out_file}")

def write_string_to_file(text_string:str, out_file:str):
    ''' 字符串文本写入文件
    @text_string: 待写入数据
    @out_file: 输出文件
    '''
    with open(out_file, "w", encoding="utf8") as f:
        f.write(text_string)
    print(f"write_string_to_file > 数据已写入文件: {out_file}")

def add_string_to_file(text_string:str, out_file:str):
    ''' 追加字符串文本到文件
    @text_string: 待追加数据
    @out_file: 输出文件
    '''
    with open(out_file, "a", encoding="utf8") as f:
        f.write("\n")
        f.write(text_string)
    print(f"add_string_to_file > 数据已追加文件: {out_file}")
